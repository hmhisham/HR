      ...
      <table class="table table-bordered table-striped">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="text-center">{{ $tenantItem->name}}</td>
            <td class="text-center">{{ $tenantItem->phone}}</td>
            <td class="text-center">{{ $tenantItem->email}}</td>
            <td class="text-center">{{ $tenantItem->address}}</td>
            <td class="text-center">{{ $tenantItem->notes}}</td>

            <td class="text-center">
              <div class="gap-1 d-flex justify-content-center">
                @can('tenant-edit')
                <button wire:click="GetTenant({{ $tenantItem->id }})" class="btn btn-icon btn-outline-success btn-sm waves-effect" data-bs-toggle="modal" data-bs-target="#edittenantModal" title="تعديل">
                  <i class="tf-icons mdi mdi-pencil fs-5"></i>
                </button>
                @endcan
                @can('tenant-delete')
                <button wire:click="GetTenant({{ $tenantItem->id }})" class="btn btn-icon btn-outline-danger btn-sm waves-effect {{ $tenantItem->active ? 'disabled' : '' }}" data-bs-toggle="modal" data-bs-target="#removetenantModal" title="حذف">
