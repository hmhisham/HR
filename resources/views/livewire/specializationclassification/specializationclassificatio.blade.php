...
<th Class="text-center">العملية</th>
  ...
  @foreach ($Specializationclassification as $Specializationclassificatio)
                           <tr>
                                    <?php $i++; ?>
                                    <td>{{ $i }}</td>
                                   <td Class="text-center">{{ $Specializationclassificatio->specializationclassification_name}}</td>

                                  <td Class="text-center">
                                     <div class="btn-group" role="group" aria-label="First group">
                                         @can('specializationclassificatio-edit')
                                             <button wire:click="GetSpecializationclassificatio({{ $Specializationclassificatio->id }})"
                                         ...
                                         @can('specializationclassificatio-delete')
                                             <button wire:click="GetSpecializationclassificatio({{ $Specializationclassificatio->id }})"
                                                 class="p-0 px-1 btn btn-outline-danger waves-effect {{ $Specializationclassificatio->active ? 'disabled' : '' }}"
